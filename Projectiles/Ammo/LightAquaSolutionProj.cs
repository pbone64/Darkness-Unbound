...
namespace DarknessUnbound.Projectiles.Ammo
{
	public class LightAquaSolutionProj : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Light Aqua Spray");
		}
		...
		{
			int dustType = ModContent.DustType<GelumSolution>();
			...
			for (int k = i - size; k <= i + size; k++)
			{
				for (int l = j - size; l <= j + size; l++)
				{
					if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size))
					{
						...
						{
							Main.tile[k, l].type = (ushort)ModContent.TileType<SoftIce_Tile>();
