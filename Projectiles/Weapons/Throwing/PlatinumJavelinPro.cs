...
{
	public class PlatinumJavelinPro : ModProjectile
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				// Create a new dust
				Dust dust = Dust.NewDustDirect(usePos, projectile.width, projectile.height, DustID.Platinum);
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
		{
			...
			target.AddBuff(BuffID.Midas, 6000); // Adds the ExampleJavelin debuff for a very small DoT
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is PlatinumJavelinPro javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
