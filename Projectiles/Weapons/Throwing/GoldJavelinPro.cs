...
{
    public class GoldJavelinPro : ModProjectile
    {
		...
		public override void Kill(int timeLeft)
		{
			...
			{
				// Create a new dust
				Dust dust = Dust.NewDustDirect(usePos, projectile.width, projectile.height, DustID.Gold);
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is GoldJavelinPro javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
