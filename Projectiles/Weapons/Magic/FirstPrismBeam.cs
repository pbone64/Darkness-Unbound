...
namespace DarknessUnbound.Projectiles.Weapons.Magic
{
	public class FirstPrismBeam : ModProjectile
	{
        ...
        private const float PiBeamDivisor = MathHelper.Pi / FirstPrismHoldout.NumBeams;
		...
		{
			DisplayName.SetDefault("First Prism Beam");
		}
		...
		public override void AI()
		{
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / FirstPrismHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > FirstPrismHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - FirstPrismHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * FirstPrismHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / FirstPrismHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
